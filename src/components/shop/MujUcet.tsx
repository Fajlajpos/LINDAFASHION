...
interface Objednavka {
  ...
  polozky: Array<{ nazev: string; slug: string; velikost: string; mnozstvi: number; cena: number }>;
}

export function MujUcet({ email, jmeno }: { email: string; jmeno: string | null }) {
  ...
  // Smazání účtu (GDPR)
  const [mazeUcet, setMazeUcet] = useState(false);
  const [potvrzujeSmazani, setPotvrzujeSmazani] = useState(false);
  const [hesloProSmazani, setHesloProSmazani] = useState('');
  ...
  const smazatUcet = async (e: React.FormEvent) => {
    e.preventDefault();
    if (mazeUcet) return;
    ...
    const vysledek = await poslatJson<{ presmerovat: string }>('/api/ucet/smazat', {
      heslo: hesloProSmazani,
    });
    ...
    setChybaSmazani(vysledek.pole?.heslo ?? vysledek.chyba);
    setMazeUcet(false);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {hlaska && (
        <p
          role="status"
          className="flex items-start gap-2 rounded-xl bg-linda-sageLight p-3 text-xs font-medium text-linda-sage"
        >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {o.lzeStornovat && (
                        <button
                          ...
                          className="flex min-h-touch cursor-pointer items-center gap-1.5 rounded-full bg-linda-cream px-4 text-xs font-semibold text-red-800 shadow-neuSm transition-all duration-200 hover:shadow-neu active:shadow-neuInsetSm disabled:opacity-60"
                        >
                          {stornujeId === o.id ? (
                            <>
      ...
      <section className="space-y-3 rounded-2xl bg-linda-cream p-6 shadow-neu">
        ...
        <p className="text-xs leading-relaxed text-linda-espresso/85">
          Smažeme vaše osobní údaje, uložené adresy, košík i oblíbené položky. Dokončené objednávky
          musíme podle zákona uchovat jako účetní doklad – zůstanou ale bez vazby na vaši osobu.
        </p>
        ...
        {potvrzujeSmazani ? (
          <form onSubmit={smazatUcet} className="space-y-3">
            <div>
              ...
              <input
                ...
                value={hesloProSmazani}
                disabled={mazeUcet}
                onChange={(e) => setHesloProSmazani(e.target.value)}
                className="min-h-touch w-full max-w-sm rounded-xl bg-linda-sandLight px-4 text-xs text-linda-espresso shadow-neuInsetSm disabled:opacity-60"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                disabled={mazeUcet}
                aria-busy={mazeUcet}
                className="flex min-h-touch cursor-pointer items-center gap-2 rounded-full bg-red-800 px-6 text-xs font-semibold text-white shadow-neuDark transition-all duration-200 hover:bg-red-900 active:shadow-neuSm disabled:opacity-70"
              >
                {mazeUcet ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />
              ...
              <button
                type="button"
                onClick={() => setPotvrzujeSmazani(false)}
                disabled={mazeUcet}
          ...
          <button
            type="button"
            onClick={() => setPotvrzujeSmazani(true)}
